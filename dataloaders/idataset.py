...
class DummyArrayDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, x, y, z=None, n_item_labels=0):

        if n_item_labels != 0:
            self.x, self.y, self.z = x, y, z
            self.n_item_labels = n_item_labels
        else:
            self.x, self.y = x, y
            self.n_item_labels =  n_item_labels
...
class iImgnet(DataHandler):
    ...
    def top_transform_function(x):
        return Image.open(x[0]).convert('RGB')
