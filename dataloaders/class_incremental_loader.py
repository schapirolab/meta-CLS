...
class IncrementalLoader:
    ...
    def new_task(self, memory=None):
        ...
        x_val, y_val = self._select(
            self.data_val, self.targets_val, low_range=min_class, high_range=max_class, subset_size=200
        )
        x_test, y_test = self._select(self.data_test, self.targets_test, low_range=min_class, high_range=max_class, subset_size=200)

        print("train size: {} {}".format(len(x_train), len(y_train)))
        print("val size: {} {}".format(len(x_val), len(y_val)))
    ...
    def _setup_test_tasks(self, validation_split):
        self.test_tasks = []
        self.val_tasks = []
        print("Inside setup test tasks")
        for i in range(len(self.increments)):
            min_class = sum(self.increments[:i])
            max_class = sum(self.increments[:i + 1])

            x_test, y_test = self._select(self.data_test, self.targets_test, low_range=min_class, high_range=max_class, subset_size=50)

            self.test_tasks.append(self._get_loader(x_test, y_test, mode="test"))

            if validation_split > 0.0:
                x_val, y_val = self._select(self.data_val, self.targets_val, low_range=min_class, high_range=max_class, subset_size=50)
    ...
    def _select(self, x, y, low_range=0, high_range=0, subset_size=-1):
        idxes = np.where(np.logical_and(y >= low_range, y < high_range))[0]
        if subset_size != -1:
            idxes_subset = np.random.choice(idxes.shape[0], subset_size, replace=False) 
            idxes = idxes[idxes_subset]
