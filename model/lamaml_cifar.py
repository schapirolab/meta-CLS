...
class Net(BaseNet):
    ...
    def forward(self, x, t, record_hidden=False):
        if record_hidden: 
            output, hidden_dict = self.net.forward(x, record_hidden=record_hidden)
            offset1, offset2 = self.compute_offsets(t)
            if offset1 > 0:
    ...
    def meta_loss(self, x, fast_weights, y, bt, t, record_sparsity=False):
        """
        ...
        offset1, offset2 = self.compute_offsets(t)
        if record_sparsity:
            logits_unsliced, proportion_active = self.net.forward(x, fast_weights, record_proportion_active=True)
            logits = logits_unsliced[:, :offset2]
    ...
    def observe(self, x, y, t, record_variables=False):
        sparsity_batch = {}
        self.net.train() 
        for pass_itr in range(self.glances):
            ...
            for i in range(n_batches):
                ...
                if record_variables:
                    meta_loss, logits, sparsity = self.meta_loss(bx, fast_weights, by, bt, t, record_sparsity=True) 
